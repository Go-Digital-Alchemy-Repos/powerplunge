...
} from "@/admin/cms-v2/presets/applyPreset.client";
...
function HistoryPanel() {
  ...
  const { data: history = [] } = useQuery<ApplyHistoryEntry[]>({
    queryKey: ["/api/admin/cms-v2/site-presets/apply-history"],
    queryFn: async () => {
      const res = await fetch("/api/admin/cms-v2/site-presets/apply-history", { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
  });
...
function ImportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const importMutation = useMutation({
    mutationFn: async (payload: unknown) => {
      const res = await fetch("/api/admin/cms-v2/site-presets/import", {
        method: "POST",
    ...
    onSuccess: (preset) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/cms-v2/site-presets"] });
...
export default function AdminCmsV2Presets() {
  ...
  const { data: presets = [], isLoading } = useQuery<SitePresetRow[]>({
    ...
    queryFn: async () => {
      const res = await fetch("/api/admin/cms-v2/site-presets", { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
  ...
  const seedMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/admin/cms-v2/site-presets/seed", {
        method: "POST",
        credentials: "include",
      });
  ...
  const duplicateMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/admin/cms-v2/site-presets/${id}/duplicate`, {
        method: "POST",
        credentials: "include",
      });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/admin/cms-v2/site-presets/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  const activateMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/cms-v2/site-presets"] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/cms-v2/site-presets/apply-history"] });
  ...
  const handleExport = async (preset: SitePresetRow) => {
    try {
      const res = await fetch(`/api/admin/cms-v2/site-presets/${preset.id}/export`, { credentials: "include" });
