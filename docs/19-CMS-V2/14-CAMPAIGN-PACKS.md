...
- **Admin URL:** `/admin/cms-v2/generator/campaigns`
...
```
client/src/admin/cms-v2/generator/campaignPacks.ts
```
...
| File | Purpose |
|------|---------|
| `client/src/admin/cms-v2/generator/campaignPacks.ts` | Campaign pack definitions (data only) |
| `client/src/admin/cms-v2/generator/assembleCampaignPack.ts` | `generateCampaignPack()` — loops through pages and calls the assembly engine |
| `client/src/pages/admin-cms-v2-generator-campaigns.tsx` | Campaign Packs UI (cards, confirmation dialog, generation log) |
| `client/src/admin/cms-v2/generator/assembleLandingPage.ts` | Assembly engine (shared with Landing Page Generator) |
...
| `client/src/components/admin/CmsV2Layout.tsx` | Sidebar navigation with Campaign Packs link |
...
Kits are resolved by name against the saved sections in the CMS. If the starter kits haven't been seeded, navigate to `/admin/cms-v2/sections` and click "Load Starter Kits" first.
