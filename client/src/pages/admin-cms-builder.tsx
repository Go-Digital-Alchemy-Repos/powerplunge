    ...
    try {
      ...
      for (const item of appState.data.content || []) {
        if (item.type === "sectionRef" && item.props?.sectionId) {
          const res = await fetch(`/api/admin/cms/sections/${item.props.sectionId}`, { credentials: "include" });
          if (res.ok) {
        ...
        <div className="flex flex-wrap gap-1" data-testid="block-category-tabs">
          {tabs.map((tab) => (
            <button
              ...
              className="px-2 py-1 rounded text-[11px] font-medium transition-colors bg-[#a0c0e8] text-[#000000]"
...
export default function AdminCmsBuilder() {
  ...
  const [, params] = useRoute("/admin/cms/pages/:id/builder");
  ...
  const handleInsertSection = useCallback(async (section: SavedSection) => {
    if (!pageId) return;
    const currentPage = queryClient.getQueryData<any>([`/api/admin/cms/pages/${pageId}`]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" data-testid="admin-cms-builder-page">
