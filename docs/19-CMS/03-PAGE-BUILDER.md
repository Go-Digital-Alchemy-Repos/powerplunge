...
The CMS page builder uses the Puck visual editor library to provide a drag-and-drop block editing experience for admin users.
...
/admin/cms/posts/:id/builder     → Post builder
```
...
```
client/src/pages/admin-cms-builder.tsx       → Page builder component
client/src/pages/admin-cms-post-builder.tsx   → Post builder component
...
5. `PUT /api/admin/cms/pages/:id` saves everything

### Loading

1. Page data fetched via `GET /api/admin/cms/pages/:id`
...
**Fix:** Create sections at `/admin/cms/sections` or seed kits via API.

## How to Verify

1. Navigate to `/admin/cms/pages/:id/builder` — editor should render
