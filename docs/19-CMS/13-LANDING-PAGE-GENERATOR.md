...
The Landing Page Generator is a 4-step wizard that assembles a complete landing page from a template, products, section kits, and page details. It produces a deterministic page via a pure assembly engine and saves it to the CMS pages system.

## Access

- **Admin URL:** `/admin/cms/generator/landing`
...
Templates are defined in `client/src/cms/templates/templateLibrary.ts`. The allowlist is enforced in `admin-cms-generator-landing.tsx` via `ALLOWED_TEMPLATE_IDS`.
...
**Slug uniqueness** is checked asynchronously via `GET /api/admin/cms/pages/check-slug?slug=`. The slug field shows a spinner while checking, a green checkmark when available, or a red warning when taken. The generate button is disabled while the slug is checking or taken.
...
```
client/src/admin/cms/generator/assembleLandingPage.ts
```
...
|--------|----------|---------|
| `POST` | `/api/admin/cms/pages` | Create the generated page |
| `POST` | `/api/admin/cms/pages/:id/publish` | Publish the page (if "Publish immediately" is on) |
| `GET` | `/api/admin/cms/pages/check-slug?slug=` | Check slug uniqueness |
| `GET` | `/api/products` | Fetch products for Step 2 |
| `GET` | `/api/admin/cms/sections` | Fetch sections/kits for Step 3 |
| `GET` | `/api/admin/cms/themes` | Fetch themes for Step 4 |

## Source Code Map

| File | Purpose |
|------|---------|
| `client/src/pages/admin-cms-generator-landing.tsx` | Wizard UI (4 steps, preview panel, all guardrails) |
| `client/src/admin/cms/generator/assembleLandingPage.ts` | Pure assembly engine (263 lines) |
| `client/src/admin/cms/generator/assembleLandingPage.test.ts` | 26 automated tests |
| `client/src/cms/templates/templateLibrary.ts` | Template definitions |
| `server/src/data/sectionKits.ts` | Section kit definitions |
| `server/src/routes/admin/cms.router.ts` | API endpoints |
| `server/src/services/cms.service.ts` | Service layer |
| `server/src/repositories/cms.repository.ts` | Database queries |
| `client/src/components/admin/CmsLayout.tsx` | Sidebar with Generators nav group |
...
The slug check queries all CMS pages (including drafts and archived pages). If a draft page already uses that slug, it will show as taken. Either choose a different slug or delete/rename the conflicting page.
...
Sections must be seeded or manually created. To seed the starter kits, use the "Load Starter Kits" button on the Sections page (`/admin/cms/sections`) or call `POST /api/admin/cms/sections/seed-kits`.
