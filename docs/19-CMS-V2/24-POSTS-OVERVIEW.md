...
The CMS v2 Posts system provides blog/article management with a full publishing workflow.
...
`shared/schema.ts` — search for `cmsV2Posts`.
...
```ts
import type { CmsV2Post, InsertCmsV2Post } from "@shared/schema";
```
...
| `server/src/services/cms-v2-posts.service.ts` | Business logic (list, create, publish, etc.) |
| `server/src/repositories/cms-v2-posts.repository.ts` | Database queries |
| `server/src/routes/admin/cms-v2-posts.routes.ts` | Admin API routes |
| `server/src/routes/public/blog.routes.ts` | Public read-only routes |
| `client/src/pages/admin-cms-v2-posts.tsx` | Admin UI for post management |

---

## Feature Flag

Posts are gated by `CMS_V2_ENABLED`. When disabled:
- Admin routes return `403 — CMS v2 is not enabled`
