...
export function SectionsContent({ embedded = false }: { embedded?: boolean }) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { name: string; description: string; category: string } }) => {
      const res = await apiRequest("PUT", `/api/admin/cms/sections/${id}`, {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => { await apiRequest("DELETE", `/api/admin/cms/sections/${id}`); },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/cms/sections"] });
  ...
  const seedKitsMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/admin/cms/sections/seed-kits");
      return res.json();
    },
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/cms/sections"] });
