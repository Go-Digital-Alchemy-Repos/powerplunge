...
function SaveDraftButton({ postId, postTitle, onDone }: { postId: string; postTitle: string; onDone: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/cms-v2/posts/${postId}`, {
        method: "PUT",
...
function PublishButton({ postId, onDone }: { postId: string; onDone: () => void }) {
  ...
  const handlePublish = async () => {
    ...
    try {
      const contentJson = puckDataToContentJson(appState.data);
      await fetch(`/api/admin/cms-v2/posts/${postId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ contentJson }),
      });
      const pubRes = await fetch(`/api/admin/cms-v2/posts/${postId}/publish`, {
        method: "POST",
        credentials: "include",
      });
...
export default function AdminCmsV2PostBuilder() {
  const { hasFullAccess, isLoading: adminLoading } = useAdmin();
  const [, params] = useRoute("/admin/cms-v2/posts/:id/builder");
  ...
  const { data: post, isLoading: postLoading } = useQuery<any>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/admin/cms-v2/posts/${postId}`, { credentials: "include" });
