...
All CMS endpoints are mounted under `/api/admin/cms` and require admin authentication with full access (via `requireFullAccess` middleware).

**Router file:** `server/src/routes/admin/cms.router.ts`

## Health

### GET /health

Returns the CMS service health status.
...
Admin menu endpoints are mounted under `/api/admin/cms/menus` via `server/src/routes/admin/cms-menus.routes.ts`.
...
All CMS admin endpoints (prefix `/api/admin/cms`):
