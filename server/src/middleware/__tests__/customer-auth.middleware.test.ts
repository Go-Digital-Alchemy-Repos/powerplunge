...
describe("customer auth middleware", () => {
  ...
  it("returns 401 without a Better Auth customer session", async () => {
    mocks.getAttachedCustomerAuthContext.mockResolvedValue(null);
  ...
  it("attaches customer context and continues for a valid Better Auth session", async () => {
    ...
    mocks.getAttachedCustomerAuthContext.mockResolvedValue(context);
    const { req, res, next } = createReqRes();

    await requireCustomerAuth(req, res, next);

    expect(mocks.getAttachedCustomerAuthContext).toHaveBeenCalledWith(req);
  ...
  it("returns 409 for merged customer accounts", async () => {
    ...
    mocks.getAttachedCustomerAuthContext.mockRejectedValue(error);
    const { req, res, next } = createReqRes();
  ...
  it("returns 503 when Better Auth is not configured", async () => {
    mocks.getAttachedCustomerAuthContext.mockRejectedValue(new Error("Better Auth is not configured for customer authentication"));
    const { req, res, next } = createReqRes();
  ...
  it("returns 403 for disabled customer accounts", async () => {
    mocks.getAttachedCustomerAuthContext.mockRejectedValue(
  ...
  it("returns 401 for unexpected Better Auth failures", async () => {
    mocks.getAttachedCustomerAuthContext.mockRejectedValue(new Error("session lookup failed"));
    const { req, res, next } = createReqRes();
