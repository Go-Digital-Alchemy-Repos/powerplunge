...
import { assembleLandingPage, type AssemblyResult } from "@/admin/cms/generator/assembleLandingPage";
...
export default function AdminCmsGeneratorLanding() {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("POST", "/api/admin/cms/pages", pageData);
      const created = await res.json();

      if (publishNow && created.id) {
        await apiRequest("POST", `/api/admin/cms/pages/${created.id}/publish`);
      }
